+++
title = "Develop Website"
date = 2020-11-28T00:10:32+08:00
tags = ["develop", "website"]
+++

* [W3C](https://www.w3.org/)
* [MDN](https://developer.mozilla.org/zh-CN/)
* [GraphQL 中文网](https://graphql.cn/)

## Mirror
* [清华大学开源软件镜像站](https://mirrors.tuna.tsinghua.edu.cn/)
* [USTC open source software mirror](http://mirrors.ustc.edu.cn/)
* [阿里云官方镜像站](https://developer.aliyun.com/mirror/)
* [东软信息学院开源镜像站](http://mirrors.neusoft.edu.cn/)
* [网易开源镜像站](http://mirrors.163.com/)
* [淘宝 NPM 镜像](https://developer.aliyun.com/mirror/NPM?from=tnpm)
* [MSDN, 我告诉你](https://msdn.itellyou.cn/)

* [Apache Hadoop](http://hadoop.apache.org/)
* [Elasticsearch 分布式、RESTful 风格的搜索和数据分析引擎](https://www.elastic.co/cn/elasticsearch/)

## Base
* [GitHub](https://github.com/)
* [Google Developers](https://developers.google.cn/)


## Server
* [Nginx](https://nginx.org/)
* [HTTP](http://httpd.apache.org/)
* [Tomcat](https://tomcat.apache.org/)

## Project
* [RabbitMQ](https://www.rabbitmq.com/)
* [Apache Kafka](http://kafka.apache.org/)

## Tool
* [Maven](https://maven.apache.org/)
* [Gradle](https://gradle.org/)
* [Jenkins](https://www.jenkins.io/zh/)
* [Zabbix](https://www.zabbix.com/)

## C
* [MinGW](http://www.mingw.org/)
* [MSYS2](https://www.msys2.org/)
* [TDM-GCC](https://jmeubank.github.io/tdm-gcc/)

## Other
* [JSON Web Tokens](https://jwt.io/)

* [Material](https://material.io/)
* [Material Design 指南中文版](https://www.mdui.org/design/)

* [网道 - 互联网开发文档](https://wangdoc.com/)

## Software
* [FileZilla](https://filezilla-project.org/)
* [XMind](https://www.xmind.net/)
* [XMind(CN)](https://www.xmind.cn/)
* [Sumatra PDF](https://www.sumatrapdfreader.org/free-pdf-reader.html)
* [electerm](https://electerm.github.io/electerm/)
* [electerm(GitHub)](https://github.com/electerm/electerm)


## 常用
+ [百度](https://www.baidu.com/)
+ [Google](https://www.google.com.hk/?gws_rd=ssl)
+ [GitHub](https://github.com/)

## 编程语言
+ [MDN Web 文档(中文)](https://developer.mozilla.org/zh-CN/)
+ [TypeScript(英文)](http://www.typescriptlang.org/)
+ [TypeScript(中文)](https://www.tslang.cn/)
+ [阮一峰 ECMAScript6 入门](http://es6.ruanyifeng.com/)
+ [Node.js(英文)](https://nodejs.org/en/)
+ [Node.js(中文)](http://nodejs.cn/)
+ [Go](https://golang.org/)
+ [Go(CN)](https://golang.google.cn/)
+ [Go(中文)](http://docscn.studygolang.com/)
+ [PHP](http://php.net/)
+ [Python](https://www.python.org/)
+ [Java](http://www.oracle.com/technetwork/java/javase/overview/index.html)
+ [Android Developers(CN)](https://developer.android.google.cn/index.html)
+ [Kotlin(英文)](http://kotlinlang.org/)
+ [Kotlin(中文)](https://www.kotlincn.net/)
+ [C/C++ 参考手册(英文)](http://en.cppreference.com/w/)
+ [C/C++ 参考手册(中文)](http://zh.cppreference.com/w/%E9%A6%96%E9%A1%B5)
+ [cplusplus.com](http://www.cplusplus.com/)
+ [Lua](http://www.lua.org/)
+ [Rust(中文)](https://prev.rust-lang.org/zh-CN/)
+ [Rust(英文)](https://www.rust-lang.org/)
+ [Dart](https://dart.dev)
+ [Gradle](https://gradle.org/)
+ [Common Lisp](https://common-lisp.net/)
+ [ANSI Common Lisp(中文)](https://acl.readthedocs.io/en/latest/zhCN/)
+ [Haskell](https://www.haskell.org/)
+ [Haskell(中文)](https://rwh.readthedocs.io/en/latest/index.html)
+ [Haskell(中文)](http://cnhaskell.com/)
+ [Racket](http://www.racket-lang.org/)
+ [Erlang](http://www.erlang.org/)
+ [Elixir](https://elixir-lang.org/)
+ [Scala](https://www.scala-lang.org/)

## 前端
### Vue
+ [Vue.js(中文)](https://cn.vuejs.org/)
+ [Vue Router(中文)](https://router.vuejs.org/zh-cn/)
+ [Vuex(中文)](https://vuex.vuejs.org/zh-cn/)
+ [Element(中文)](http://element-cn.eleme.io/#/zh-CN)
+ [Weex(中文)](http://weex.apache.org/cn/)

### Angular
+ [Angular(英文)](https://angular.io/)
+ [Angular(中文)](https://www.angular.cn/)

### React
+ [React(英文)](https://reactjs.org/)
+ [React(中文)](https://reactnative.cn/)
+ [React Native(英文)](http://facebook.github.io/react-native/)

### Flutter
+ [Flutter](https://flutter.dev/)

## 后端框架
### Nodejs
+ [Express(中文)](http://www.expressjs.com.cn/)
+ [Koajs(英文)](http://koajs.com/)
+ [Koajs(中文)](https://koa.bootcss.com/)
+ [eggjs(中文)](https://eggjs.org/)
+ [Mongoose](https://mongoosejs.com/)

### Java
+ [Spring](https://spring.io/)
+ [Struts](https://struts.apache.org/)


### PHP
+ [ThinkPHP](http://www.thinkphp.cn/)
+ [CodeIgniter(CN)](http://codeigniter.org.cn/)

### Python
+ [Scrapy](https://scrapy.org/)
+ [Django](https://www.djangoproject.com/)


## Develop
+ [Apache](https://www.apache.org/)
+ [Steel Bank Common Lisp](http://www.sbcl.org/)


## Design
+ [Material Design(EN)](https://material.io/)
+ [Material Design(CN)](http://design.1sters.com/)
+ [Ant Design](https://ant.design/index-cn)
+ [Dribbble](https://dribbble.com/)
+ [优设](http://www.uisdc.com/)

## 编辑器
### 编辑器官网
+ [Emacs](http://www.gnu.org/software/emacs/)
+ [VS Code](https://code.visualstudio.com/)
+ [Atom](https://atom.io/)
+ [Vim](http://www.vim.org/)
+ [Notepad++](https://notepad-plus-plus.org/)
+ [CodeLite](https://codelite.org/)
+ [Brackets](http://brackets.io/)
+ [Sublime](http://www.sublimetext.com/)
+ [Komodo Edit](https://www.activestate.com/komodo-ide/downloads/edit)
+ [JetBrains](https://www.jetbrains.com/)
+ [IntelliJ IDEA](https://www.jetbrains.com/idea/)
+ [PyCharm](https://www.jetbrains.com/pycharm/)
+ [Genuitec](https://www.genuitec.com/)
+ [Angular IDE](https://www.genuitec.com/products/angular-ide/)
+ [Android Studio(CN)](https://developer.android.google.cn/studio/index.html)
+ [eclipse](http://www.eclipse.org/downloads/)
+ [EmEditor](https://www.emeditor.com/)
+ [Visual Studio](https://www.visualstudio.com/zh-hans/downloads/)
+ [EditPlus](https://www.editplus.com/)

### 其他编辑器
+ [VNote](https://tamlok.github.io/vnote/)

### Vim插件
+ [vim-javascript(GitHub)](https://github.com/pangloss/vim-javascript)
+ [vim-typescript(GitHub)](https://github.com/leafgarland/typescript-vim)
+ [vim-go(GitHub)](https://github.com/fatih/vim-go)
+ [vim-emmet(GitHub)](https://github.com/mattn/emmet-vim)
+ [vim-vue(GitHub)](https://github.com/posva/vim-vue)
+ [vim-one colorschemes(GitHub)](https://github.com/rakr/vim-one)
+ [vim-solarized colorschemes(GitHub)](https://github.com/altercation/vim-colors-solarized)
+ [Vim Colors](http://vimcolors.com/)

### Emacs插件
+ [org-mode](https://orgmode.org/)
+ [spacemacs(GitHub)](https://github.com/syl20bnr/spacemacs)
+ [go-model(GitHub)](https://github.com/dominikh/go-mode.el)
+ [web-mode(GitHub)](https://github.com/fxbois/web-mode)
+ [GNU AUCTeX](http://www.gnu.org/software/auctex/)
+ [Slime(GitHub)](https://github.com/slime/slime)
+ [Slime](https://common-lisp.net/project/slime/)
+ [Racer(GitHub)](https://github.com/racer-rust/emacs-racer)
+ [Vue-mode(GitHub)](https://github.com/AdamNiederer/vue-mode)
+ [Company-mode](http://company-mode.github.io/)
+ [Magit](https://magit.vc/)
+ [Emacssmirror(GitHub)](https://github.com/emacsmirror)
+ [MELPA](https://melpa.org/#/)
+ [ElPA](http://elpa.gnu.org/)
+ [HideShow-mode](https://www.emacswiki.org/emacs/HideShow)
+ [ido-mode](https://www.emacswiki.org/emacs/InteractivelyDoThings)
+ [RefTex](https://www.gnu.org/software/emacs/manual/html_mono/reftex.html)

### Emacs Document
+ [Emacs Document](https://www.gnu.org/software/emacs/documentation.html)
+ [ErgoEmacs](http://ergoemacs.org/)
+ [AucTeX](https://www.gnu.org/software/auctex/)


## 普通文本标记语言
+ [Org mode](https://orgmode.org/)
+ [Org mode Manual](https://orgmode.org/org.html)
+ [Org mode Syntax](https://orgmode.org/worg/dev/org-syntax.html)
+ [Org mode Manual(中文)](http://orgmode-cn.marboo.io/org.html)
+ [Org mode Manual(emacs)](http://www.gnu.org/software/emacs/manual/html_node/org/)
+ [GitHub Markdown](https://guides.github.com/features/mastering-markdown/)
+ [Markdown 语法(中文CSDN)](http://blog.csdn.net/witnessai1/article/details/52551362)
+ [GNU Texinfo](http://www.gnu.org/software/texinfo/)
+ [LaTeX](https://www.latex-project.org/)

## Develop
+ [Google Developer(CN)](https://developers.google.cn/)
+ [W3C](https://www.w3.org/)
+ [GraphQL(CN)](http://graphql.cn/)

## Fonts
+ [Google Fonts](https://fonts.google.com/)
+ [Google Fonts Inconsolata](https://fonts.google.com/specimen/Inconsolata?selection.family=Inconsolata)
+ [Source Code Pro(GitHub)](https://github.com/adobe-fonts/source-code-pro)


## software
+ [Mongodb](https://www.mongodb.com/)
+ [Memcached](http://memcached.org/)
+ [Redis](https://redis.io/)
+ [MySQL](https://www.mysql.com/)
+ [PostgreSQL](https://www.postgresql.org/)
+ [FileZilla](https://filezilla-project.org/)
+ [XMind](http://www.xmind.net/)
+ [Cygwin](https://cygwin.com/index.html)
+ [Git](https://git-scm.com/)
+ [Postman](https://www.getpostman.com/)
+ [Sumatra PDF](https://www.sumatrapdfreader.org/free-pdf-reader.html)


## Linux
+ [Linux工具快速教程](https://linuxtools-rst.readthedocs.io/zh_CN/latest/index.html)
+ [Linux就该这么学](https://www.linuxprobe.com/)
+ [Linux Kernel](https://www.kernel.org/)
+ [Docker从入门到实践](https://yeasy.gitbooks.io/docker_practice/content/)
+ [Docker从入门到实践](https://legacy.gitbook.com/book/yeasy/docker_practice/details)


## Golang Packages
+ [gocode](https://github.com/nsf/gocode)
+ [godef](https://github.com/rogpeppe/godef)


